...
void solve(){
    ...
    for(int i=0; i<num_toys; i++){
        ...
        while(l<r){
            int m = (l+r)/2;
            //cout<<partitions[m].first<<", "<<partitions[m].second<<": "<<toys[i].first<<", "<<toys[i].second<<endl;
            if(on_right(partitions[m].first, y_1, partitions[m].second, y_2, toys[i].first, toys[i].second)){ 
                //cout<<"on_right true"<<endl;
                l = m+1;
            }
            else {
                //cout<<"on_right false"<<endl;
                r=m;
            }
        }
